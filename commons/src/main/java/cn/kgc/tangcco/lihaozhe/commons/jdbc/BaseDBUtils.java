...
public abstract class BaseDBUtils {
	...
	public static PreparedStatement getPreparedStatement(Connection conn, String sql, Laypage laypage) throws SQLException {
		// MySQL分页中每页的起始index
		// int index = (pr.getPageNumber() - 1) * pr.getPageSize();
		// sql += " limit " + index + " , " + pr.getPageSize();
		sql += " limit " + (laypage.getPage() - 1) * laypage.getLimit() + " , " + laypage.getLimit();
		System.out.println(sql.toString());
		// 在该连接中预编译SQL
		return conn.prepareStatement(sql);
	}
