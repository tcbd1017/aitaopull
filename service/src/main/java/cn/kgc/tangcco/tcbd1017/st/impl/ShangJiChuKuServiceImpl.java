...
public class ShangJiChuKuServiceImpl implements ShangJiChuKuService{
	...
	public Map<String, Object> ChaXunPinPai(Map<String, Object> map) {
		...
		if (map!=null) {
			...
			List<Brand> list = brandDaoImpl.selectPinPai(map);
	...
	@Override
	 public Map<String, Object> ChaXunLeiXing() {
	  ...
	  List<Type> list = typeDao.selectLeiXing();
	  map1.put("status", "success");
	  map1.put("data", list);
	//  if (map!=null) {
	//   TypeDaoImpl typeDao = new TypeDaoImpl();
	 ...
	 public Map<String, Object> selectAllChuku(Map<String, Object> map) {
	  ...
	  if (map!=null) {
	   try {
	    BaseDBUtils.startTransaction();
	    ChuKuDaoImpl chuKuDaoImpl=new ChuKuDaoImpl();
	   List<Chuku> selectChuKu = chuKuDaoImpl.selectChuKu(map);
	    if (selectChuKu!=null) {
	     map1.put("status", "success");
	     map1.put("data", selectChuKu);
	     BaseDBUtils.commitAndClose();
	    }
	    return map1;
	   } catch (SQLException e) {
